...
export default function LearningPaths({ skillGaps, targetRole }) {
  ...
  const generateLearningPath = async () => {
    ...
    setLoading(true);
    try {
      const prompt = `You are a learning strategist. Create a MINIMAL, bundled learning path for someone targeting a ${targetRole} role.

Their skill gaps are: ${skillGaps.join(", ")}

CRITICAL REQUIREMENTS:
1. Create 3-4 SKILL PACKAGES (not individual courses). Each package groups related skills.
2. Each package should take 4-8 weeks to complete (not 6 months).
3. For each package, recommend ONLY 1-2 courses maximum (the most essential ones).
4. For each package, suggest 1 capstone project that proves ALL skills in the package.
5. Include 1 optional certification per package (if truly valuable for the role).
6. Order packages by priority for landing this role.

EXAMPLE STRUCTURE:
- Package 1: "SQL & Database Fundamentals" (high priority)
  - Skills: SQL, Database Design, Data Querying
  - Core Course: "SQL for Data Analysis" (Coursera, 3 weeks)
  - Capstone: Build a database-backed analysis tool
  - Certification: AWS Certified Cloud Practitioner (optional but valuable)

Return only the most essential learning path to get job-ready, not comprehensive education.`;
            ...
            packages: {
              ...
              items: {
                ...
                properties: {
                  ...
                  skills_in_package: { type: "array", items: { type: "string" } },
      ...
      setCourses(result.packages || []);
      setTotalWeeks(result.total_timeline_weeks || null);
    } catch (error) {
