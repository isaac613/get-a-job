...
ROLE LIBRARY (use to understand what the target role requires — responsibilities, required skills, keywords):
...
PROOF SIGNAL LIBRARY (use to identify which of the user's experiences are strongest proof signals for the target role — and surface them prominently in the CV):
${JSON.stringify(proofSignalLibrary, null, 2)}

ROLE-SKILL MAPPING (use to know which skills are core, secondary, and differentiator for the target role — lead with core skills in the CV):
...
- Use keywords from the job description and role library naturally — do not keyword-stuff
    ...
    if (Array.isArray(cvData.education) && cvData.education.length > 0) {
      ...
      (cvData.education as any[]).forEach((edu) => {
        ...
        if (edu.details?.length > 0) {
          addSpace(2);
          edu.details.forEach((d: string) => addText(`• ${d}`, 9));
        }
        addSpace(5);
      });
    }
